...
async function runBoundaryTests() {
  ...
  assert(pkgSite1NoZone.statusCode === 201 && pkgSite1NoZone.body.data?.id, '站点1无区域包裹创建成功');
  ...
  assert(pkgSite2NoZone.statusCode === 201 && pkgSite2NoZone.body.data?.id, '站点2无区域包裹创建成功');
  ...
  assert(pkgNoSiteNoZone.statusCode === 201 && pkgNoSiteNoZone.body.data?.id, '无站点无区域包裹创建成功');
  ...
  assert(pkgFullInfo.statusCode === 201 && pkgFullInfo.body.data?.id, '完整信息包裹创建成功');
  ...
  assert(pkgCrossZone.statusCode === 201 && pkgCrossZone.body.data?.id, '跨区域包裹创建成功');
  ...
  assert(previewSite1.statusCode === 200, '预览接口返回 200');
  const site1Courier = previewSite1.body.data?.details?.[0]?.recommended_courier;
  assert(site1Courier, '站点1无区域包裹有推荐小哥');
  ...
  assert(previewSite2.statusCode === 200, '预览接口返回 200');
  const site2Courier = previewSite2.body.data?.details?.[0]?.recommended_courier;
  assert(site2Courier, '站点2无区域包裹有推荐小哥');
  ...
  const previewGlobal = await makeRequest('POST', '/api/packages/dispatch/preview', {
    package_ids: [createdPackageIds[2]],
  });
  assert(previewGlobal.statusCode === 200, '预览接口返回 200');
  const globalCourier = previewGlobal.body.data?.details?.[0]?.recommended_courier;
  assert(globalCourier, '无站点无区域包裹有推荐小哥');
  ...
  if (site1Courier && site2Courier) {
    ...
    assert(different, '站点1和站点2推荐小哥不同');
  } else {
    assert(false, '无法验证站点隔离，缺少推荐小哥数据');
  }
  ...
  assert(r1.statusCode === 400, '空请求体返回 400');
  ...
  assert(r2.statusCode === 400, 'package_ids 为 null 返回 400');
  ...
  assert(r3.statusCode === 400, 'package_ids 为 undefined 返回 400');
  ...
  assert(r4.statusCode === 400, 'package_ids 不是数组返回 400');
  ...
  assert(r5.statusCode === 400, 'package_ids 是空数组返回 400');
  ...
  assert(r6.statusCode === 200, 'operator_name 为数字正常处理（自动转字符串）');
  ...
  assert(r7.statusCode === 200, '包含 null/undefined 的数组正常处理');
  assert(r7.body.data?.summary?.total === 4, '总数为 4');
  assert(r7.body.data?.summary?.data_error >= 2, '至少 2 个数据异常');
  console.log('  状态码:', r7.statusCode);
  console.log('  处理后总数:', r7.body.data?.summary?.total);
  console.log('  成功数:', r7.body.data?.summary?.success);
  console.log('  数据异常:', r7.body.data?.summary?.data_error);

  console.log('\n3.7.1 测试仅包含 null 的 ID 数组...');
  const r7OnlyNull = await makeRequest('POST', '/api/packages/dispatch/preview', {
    package_ids: [null],
  });
  assert(r7OnlyNull.statusCode === 200, '仅包含 null 的数组正常处理');
  assert(r7OnlyNull.body.data?.summary?.total === 1, '总数为 1');
  assert(r7OnlyNull.body.data?.summary?.data_error === 1, '数据异常为 1');
  console.log('  状态码:', r7OnlyNull.statusCode);
  console.log('  处理后总数:', r7OnlyNull.body.data?.summary?.total);
  console.log('  数据异常:', r7OnlyNull.body.data?.summary?.data_error);
  ...
  assert(r8.statusCode === 200, '包含重复 ID 的数组正常处理');
  assert(r8.body.data?.summary?.total === 1, '去重后总数为 1');
  console.log('  状态码:', r8.statusCode);
  console.log('  去重后总数:', r8.body.data?.summary?.total);
  ...
  const confirmBefore = await makeRequest('POST', '/api/packages/dispatch/confirm', {
    package_ids: [pkgForConfirm],
    operator_name: '边界测试人员',
  });
  assert(confirmBefore.statusCode === 200, '确认分派返回 200');

  const previewAfterConfirm = await makeRequest('POST', '/api/packages/dispatch/preview', {
    package_ids: [pkgForConfirm],
  });
  assert(previewAfterConfirm.statusCode === 200, '确认后预览返回 200');
  const afterDetail = previewAfterConfirm.body.data?.details?.[0];
  assert(afterDetail?.type === 'SKIPPED', '已分配包裹被跳过');
  console.log('  包裹状态:', afterDetail?.package_info?.status);
  console.log('  结果类型:', afterDetail?.type);
  console.log('  原因:', afterDetail?.reason);

  console.log('\n4.3 重复确认同一包裹 - 第二次应该被跳过...');
  const confirm1 = await makeRequest('POST', '/api/packages/dispatch/confirm', {
    package_ids: [createdPackageIds[1]],
  });
  assert(confirm1.statusCode === 200, '第一次确认返回 200');
  const confirm2 = await makeRequest('POST', '/api/packages/dispatch/confirm', {
    package_ids: [createdPackageIds[1]],
  });
  assert(confirm2.statusCode === 200, '第二次确认返回 200');
  assert(confirm2.body.data?.summary?.skipped >= 1, '第二次确认至少跳过 1 个');
  console.log('  第一次确认成功数:', confirm1.body.data?.summary?.success);
  console.log('  第二次确认成功数:', confirm2.body.data?.summary?.success);
  console.log('  第二次跳过数:', confirm2.body.data?.summary?.skipped);
  ...
  assert(racePreview.statusCode === 200, '预览返回 200');
  ...
  assert(raceConfirm.statusCode === 200, '确认返回 200');
  const confirmDetail = raceConfirm.body.data?.details?.find(d => d.package_id === pkgRace);
  assert(confirmDetail?.type === 'SKIPPED', '状态变更后确认被跳过');
  console.log('  确认时结果类型:', confirmDetail?.type);
  ...
  assert(mixedPreview.statusCode === 200, '混合预览返回 200');
  assert(mixedPreview.body.data?.summary?.total === 7, '总数为 7');
  ...
  assert(mixedConfirm.statusCode === 200, '混合确认返回 200');
  ...
  assert(pkgDetail.statusCode === 200, '获取包裹详情返回 200');
  console.log('\n6.1 验证确认分派后的轨迹记录...');
  ...
  assert(pkgDetail.body.data?.tracks?.length >= 1, '确认分派后轨迹记录 >= 1');
...
async function main() {
  try {
    const success = await runBoundaryTests();
