...
async function testConflictCheck() {
  ...
  assert(appt1 && appt1.tracking_no, '包裹 3 预约创建成功');
  ...
  try {
    ...
    assert(false, '同一时间段冲突应该被拦截');
    console.log('✗ 应该失败但成功了');
  } catch (err) {
    assert(err && err.message && err.message.includes('冲突'), '正确拦截冲突预约');
  ...
  try {
    ...
    assert(false, '部分时间重叠应该被拦截');
    console.log('✗ 应该失败但成功了');
  } catch (err) {
    assert(err && err.message && err.message.includes('冲突'), '正确拦截部分重叠');
  ...
  try {
    ...
    assert(false, '完全包含时间段应该被拦截');
    console.log('✗ 应该失败但成功了');
  } catch (err) {
    assert(err && err.message && err.message.includes('冲突'), '正确拦截包含关系');
  ...
  assert(appt2 && appt2.tracking_no, '不冲突时段预约创建成功');
  ...
  assert(updated && updated.tracking_no, '修改自身预约成功（排除自身）');
  ...
  try {
    ...
    assert(false, '修改到冲突时间段应该被拦截');
    console.log('✗ 应该失败但成功了');
  } catch (err) {
    assert(err && err.message && err.message.includes('冲突'), '正确拦截修改冲突');
  ...
  assert(!conflictAfterCancel.hasConflict, '取消后同一时段不再冲突');
  ...
  assert(appt4 && appt4.tracking_no, '未分配小哥的包可以创建预约（不检查冲突）');
...
async function main() {
  let success = false;
  try {
    success = await testConflictCheck();
  } catch (err) {
